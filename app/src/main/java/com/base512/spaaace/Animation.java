...
public class Animation {
	...
	public Animation(int columns, int rows) {
		...
		//Log.d("Spaaace", "Animation constructor");
	...
	private void findCollisions() {
		//Log.i("Spaaace", "Animation: find collisions");
		...
		for (Entity e : list) {
			...
			for (Entity f : list) {
				...
				//Log.i("Spaaace", "Animation: " + e.name + " collided with " + f.name);
				e.collisions.add(f);
			}
		}
	}
	...
	private void buildScreen() {
		...
		for(Entity star : getEntitiesOfType(Spaaace.ENTITY_TYPE_STAR)) {
			star.draw(screen, star.frame);
		}
		for(Entity planet : getEntitiesOfType(Spaaace.ENTITY_TYPE_PLANET)) {
			planet.draw(screen, planet.frame);
		}
		for(Entity everything : getEntitiesOfType(-1)) {
			everything.draw(screen, everything.frame);
		}
	}
	...
	private void removeDeletedEntities() {
		...
		for (Entity e : delList) {
			if (e.deathCallback != null) {
				//Log.i("Spaaace", "Animation: invoke death callback for " + e.name);
	...
	private void doCallbacks() {
		//Log.i("Spaaace", "Animation: do callbacks");
		for (Entity e : list) {
			
			//Log.i("Spaaace", "Animation: do " + e.name + " callbacks");
			...
			if (e.dieFrame > 0) {
				if (e.frameCount >= e.dieFrame) {
					//Log.i("Spaaace", "Animation: " + e.name + " dies at frame " + e.frame);
					delEntity(e);
				}
			}
			...
			if (e.dieOffscreen) {
				if (e.fx >= screen.columns || e.fy >= screen.rows ||
					...
					//Log.i("Spaaace", "Animation: " + e.name + " dies offscreen");
					delEntity(e);
					continue;
				}
			}
